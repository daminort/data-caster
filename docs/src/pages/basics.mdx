...
import { basicInit, boolExample } from '../code/common';
...
This option is used to determine whether transform object's properties unto `camelCase`-notation
or not.
...
> Note:
> If object's property end with `_id` or `_Id` and `forceCamelCase === true` the result will be
...
More examples you can get in the [API reference](/api-reference) section.
